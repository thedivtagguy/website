...
<a href={url}  rel={isExternal ? "" : 'external'}>
  ...
  >
    {#if !image}
      ...
      />
    {:else}
      <img width="640" height="500" src={image} alt="thumbnail for story" />
    {/if}
    ...
    <div class="flex min-w-[70px] flex-col gap-2">
      {#each keyword as tag, i}
        {#if i < 2}
          <p
            class="text-sm capitalize text-center font-semibold text-white rounded-sm border-[3px] border-black bg-gray-700"
          >
            {tag}
          </p>
        {/if}
    ...
    <p
      class="font-bold mx-auto self-center rotate-90 font-serif text-center text-2xl  min-w-[100px] max-w-[150px] border-b-2 border- border-black"
    >
      {day} / {monthShort}
    </p>
  </div>
