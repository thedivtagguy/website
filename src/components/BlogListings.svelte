<script>
  ...
  <main>
    ...
    <section class="flex lg:h-[150px] sm:h-[150px] md:h-[150px] xl:h-[150px] pb-4 justify-between items-center mx-auto">
      <h1 class="text-5xl lg:text-6xl mx-auto text-white font-medium font-serif text-center py-4">Yet Another Blog</h1>
    </section>
    <section class="z-2 py-4 bg-white">
      <h2 class="text-4xl font-medium font-serif text-left border-b-2 border-black pb-2 mb-2">Data Storytelling</h2>
      <section id="data" class="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3  gap-4 py-4">
        ...
        {#each refined as project}
          {#if project.category.includes("blogpost")}
            <div>
              <StoryCard {...project} />
            </div>
          {/if}
